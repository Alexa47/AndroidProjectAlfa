...
public class NiemieckiPodst extends AppCompatActivity {
    ...
    private void getNewWord(){
        ...
        getSupportFragmentManager().beginTransaction().replace(R.id.My_Container_1_ID_N, fragment1).commit();
        getSupportFragmentManager().beginTransaction().replace(R.id.My_Container_2_ID_N, fragment2).commit();
        if(temp.length==3){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_niemiecki_podst);
        ...
        getSupportFragmentManager().beginTransaction().replace(R.id.My_Container_1_ID_N, fragment1).commit();
        getSupportFragmentManager().beginTransaction().replace(R.id.My_Container_2_ID_N, fragment2).commit();
        image=findViewById(R.id.wordImageN);
        if(temp.length==3){
            String file=temp[2];
            String PACKAGE_NAME=getApplicationContext().getPackageName();
