...
public class RozszerzenieAngielski extends AppCompatActivity {
    ...
    private void getNewWord(){
        ...
        translation=temp[0];
        getSupportFragmentManager().beginTransaction().replace(R.id.cont1, fragment1).commit();
        if(temp.length==3){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_rozszerzenie_angielski);
        ...
        //fragment2=new Fragment_b();
        getSupportFragmentManager().beginTransaction().replace(R.id.cont1, fragment1).commit();
        //getSupportFragmentManager().beginTransaction().replace(R.id.cont2, fragment2).commit();
        sm=(SensorManager) getSystemService(Context.SENSOR_SERVICE);
        sm.registerListener(sensorListener, sm.getDefaultSensor(Sensor.TYPE_ACCELEROMETER), SensorManager.SENSOR_DELAY_NORMAL);
        acelVal=SensorManager.GRAVITY_EARTH;
        acelLast=SensorManager.GRAVITY_EARTH;
        shake=0.00f;
        image=findViewById(R.id.rozWordImage);
        if(temp.length==3){
            String file=temp[2];
            String PACKAGE_NAME=getApplicationContext().getPackageName();
